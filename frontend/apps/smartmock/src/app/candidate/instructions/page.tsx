...
import { GenerationProgress, LoadingSpinner } from "@/components/AssessmentStatusComponents"
...
export default function Instructions() {
  ...
  // Phase 6: Assessment readiness check
  const { readiness, loading: readinessLoading, error: readinessError, refresh } = useAssessmentReadiness(testId)
  ...
  const startAssessment = async () => {
    // Phase 6: Verify readiness before starting
    if (!readiness?.ready) {
      alert("Assessment is not ready yet. Please wait for questions to be generated.")
      return
    }

    if (readiness.status === "generation_failed") {
      alert("Question generation failed. Please contact support or try refreshing.")
      return
    }

    if (readiness.status === "partially_generated") {
      const confirm = window.confirm(
        `Only ${readiness.ready_questions} out of ${readiness.total_questions} questions were generated. Do you want to continue with a partial assessment?`
      )
      if (!confirm) return
    }
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-6">
          <AnimateOnScroll animation="fadeInUp" delay={200}>
            <div className="bg-white/95 backdrop-blur-sm border border-warm-brown/10 rounded-2xl p-8 max-w-3xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
              {/* Phase 6: Assessment readiness check UI */}
              {readinessLoading ? (
                <div className="text-center py-12">
                  <LoadingSpinner message="Checking assessment readiness..." />
                </div>
              ) : readinessError ? (
                <div className="text-center py-12">
                  <div className="text-red-600 mb-4">
                    ...
                    <p className="text-sm text-gray-600 mb-4">{readinessError}</p>
                    <Button onClick={refresh} variant="outline">Retry</Button>
                  </div>
                </div>
              ) : readiness?.status === "generating" ? (
                <div className="text-center py-12">
                  <GenerationProgress
                  ...
                  <p className="text-sm text-gray-500 mt-6">
                    Please wait while we prepare your assessment. This usually takes 1-2 minutes.
                  </p>
                </div>
              ) : readiness?.status === "generation_failed" ? (
                <div className="text-center py-12">
                  <div className="text-red-600 mb-4">
                    <svg className="w-16 h-16 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <h3 className="text-xl font-medium mb-2">Question Generation Failed</h3>
                    <p className="text-sm text-gray-600 mb-4">{readiness.message}</p>
                    {readiness.retry_recommended && (
                      <Button onClick={refresh}>Retry Generation</Button>
                    )}
                ...
                <>
                  {/* Phase 6: Show generation status if partially generated */}
                  {readiness?.status === "partially_generated" && (
                    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <p className="text-sm font-medium text-yellow-800">Partial Assessment</p>
                          <p className="text-xs text-yellow-700 mt-1">
                            {readiness.ready_questions} of {readiness.total_questions} questions are ready. You can proceed, but the assessment may be incomplete.
                          </p>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="text-center">
                    <Button
                      ...
                      disabled={isStarting || !readiness?.ready}
                      size="lg"
                    ...
                    {/* Phase 6: Show readiness hint */}
                    {!readiness?.ready && readiness && (
                      <p className="text-xs text-gray-500 mt-3">{readiness.message}</p>
                    )}
                  </div>
                </>
              )}
