...
export default function ReportsPage() {
    ...
    const { activeContext } = useAdminContext()
    ...
    const fetchReports = useCallback(async () => {
        try {
            ...
            // Fetch submissions (tests) and assessments in parallel (context-aware)
            const [subRes, assessRes] = await Promise.all([
                fetch(buildApiUrl('/api/admin/tests'), { headers: { Authorization: `Bearer ${token}` } }),
                fetch(buildApiUrl('/api/admin/assessments'), { headers: { Authorization: `Bearer ${token}` } })
            ])
