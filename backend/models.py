...
class ScoringStatus(str, Enum):
    """Status of the AI scoring process for a submission"""
    PENDING = "pending"  # Submission completed, waiting for scoring
    IN_PROGRESS = "in_progress"  # Scoring is currently running
    COMPLETED = "completed"  # Scoring finished successfully
    FAILED = "failed"  # Scoring encountered an error
    SKIPPED = "skipped"  # Scoring was skipped (e.g., no scoreable questions)
...
class Submission(CosmosDocument):
    ...
    # AI Scoring Status Tracking (Phase 1)
    scoring_status: Optional[ScoringStatus] = Field(None, alias="scoringStatus", description="Status of AI scoring process")
    scoring_started_at: Optional[datetime] = Field(None, alias="scoringStartedAt", description="When scoring started")
    scoring_completed_at: Optional[datetime] = Field(None, alias="scoringCompletedAt", description="When scoring completed")
    scoring_error: Optional[str] = Field(None, alias="scoringError", description="Error message if scoring failed")
    scoring_method: Optional[str] = Field(None, alias="scoringMethod", description="Method used for scoring (e.g., 'autogen_v1', 'hybrid_v1')")
...
class UpdateSubmissionRequest(BaseModel):
    ...
    auto_submit_timestamp: Optional[str] = Field(None, alias="autoSubmitTimestamp", description="When auto-submission occurred")
...
class AssessmentReport(CosmosDocument):
    """Comprehensive assessment report generated by Autogen agents (stored in Reports container)"""
    model_config = ConfigDict(populate_by_name=True, arbitrary_types_allowed=True)
    ...
    # Report content
    executive_summary: str = Field(..., alias="executiveSummary", description="High-level overview")
    detailed_analysis: str = Field(..., alias="detailedAnalysis", description="Full question-by-question analysis")
    recommendations: List[str] = Field(default_factory=list, description="Actionable recommendations")
    
    # Structured data
    competency_breakdown: Optional[Dict[str, Any]] = Field(None, alias="competencyBreakdown")
    strengths: List[str] = Field(default_factory=list, description="Top strengths identified")
    areas_for_development: List[str] = Field(default_factory=list, alias="areasForDevelopment")
    
    # Metadata
    generation_method: str = Field("autogen_report_v1", alias="generationMethod")
    generated_at: datetime = Field(default_factory=now_ist, alias="generatedAt")
    generation_duration_seconds: Optional[float] = Field(None, alias="generationDurationSeconds")
    agent_conversation_length: Optional[int] = Field(None, alias="agentConversationLength")
    
    # Scoring reference
    evaluation_id: Optional[str] = Field(None, alias="evaluationId", description="Reference to evaluation record")
    overall_score: Optional[float] = Field(None, alias="overallScore")
    max_possible_score: Optional[float] = Field(None, alias="maxPossibleScore")
    percentage_score: Optional[float] = Field(None, alias="percentageScore")
    
    # Raw data (optional, for debugging)
    raw_report: Optional[str] = Field(None, alias="rawReport", description="Unprocessed agent output")
    
    created_at: datetime = Field(default_factory=now_ist, alias="createdAt")
    updated_at: datetime = Field(default_factory=now_ist, alias="updatedAt")
