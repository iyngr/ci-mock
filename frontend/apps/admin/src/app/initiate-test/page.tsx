...
export default function InitiateTest() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(buildApiUrl('/api/admin/tests'), {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${adminToken}`,
          // Attach logical product source so backend persists correct document source
          "X-Source": getActiveAdminSource()
        },
        body: JSON.stringify({
      ...
      if (data.success) {
        ...
        if (data.source) setCreatedSource(data.source)
        setSuccess(true)
      } else {
          ...
          <AnimateOnScroll animation="fadeInUp" delay={200}>
            ...
            <div className="bg-white/60 backdrop-blur-sm border border-warm-brown/10 rounded-2xl p-8 text-center">
              ...
              <div className="bg-warm-brown/5 border border-warm-brown/10 rounded-xl p-6 mb-8">
                ...
                {createdSource && (
                  <div className="mt-3 text-sm text-warm-brown/70">
                    <span className="font-medium">Persisted source:</span> {createdSource}
                  </div>
                )}
              </div>
